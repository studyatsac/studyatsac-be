...
module.exports = function (lang) {
    return Joi.object({
        ...
        interviewSectionAnswers: Joi.array().items(UserInterviewSectionAnswerValidation(lang)).required().error(new Error(lang.USER_INTERVIEW_SECTION.INTERVIEW_SECTION_ANSWERS_NOT_VALID)),
        review: Joi.string().max(40000).allow('', null).optional().error(new Error(lang.USER_INTERVIEW_SECTION.REVIEW_NOT_VALID)),
        language: Joi.string().valid(...Object.values(require('../../../constants/common').LANGUAGE)).optional().error(new Error(lang.USER_INTERVIEW_SECTION.LANGUAGE_NOT_VALID))
    });
};
