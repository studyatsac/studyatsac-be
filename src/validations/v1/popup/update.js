...
module.exports = function (lang) {
    return Joi.object({
        ...
        link_url: Joi.string().uri().max(500).optional()
            .error(new Error('Link URL must be a valid URL and not exceed 500 characters')),

        start_date: Joi.date().optional().allow(null)
            .error(new Error('Start date must be a valid date')),

        end_date: Joi.date().optional().allow(null)
            .error(new Error('End date must be a valid date')),
