...
exports.continueMockInterview = async (req, res) => {
    try {
        ...
        const result = await MockInterviewService.continueMockInterview({ uuid, userId }, { lang });
        if (!result.status) {
        ...
        LogUtils.logError({
            functionName: 'continueMockInterview',
            message: err.message
        });
