...
const getMyExamPackage = async (input, opts = {}) => {
  ...
  const userPurchase =
    await UserPurchaseRepository.findWithExamPackageAndCategoryAndCountAll(
      whereClause,
      optionsClause
    );
  ...
  const data = {
    rows: [...userPurchaseMap.values()],
    count: userPurchase.count,
  };
...
const getMyExam = async (input, opts = {}) => {
  ...
  whereClauseExam.examPackageIds = userPurchases.map(
    (userPurchase) => userPurchase.examPackageId
  );

  const examPackageMappings =
    await ExamPackageMappingRepository.findAllWithExam(whereClauseExam);
...
const getUserPurchaseEssayPackageList = async (input, opts = {}) => {
  ...
  if (!allEssayPackage) {
    return Response.formatServiceReturn(
      ...
      404,
      null,
      language.USER_PURCHASE.NOT_FOUND
    );
  }
...
const injectUserPurchase = async (input, opts = {}) => {
  ...
  const userPurchased = await UserPurchaseRepository.create(
    userPurchasePayload
  );

  const paidProduct = await ProductRepository.findOne({
    examPackageId: examPackage.id,
  });

  if (paidProduct) {
    const hotsPackageActive =
      await UserPurchaseRepository.findOneExcludeExpired({
        userId: user.id,
...
const createUserPurchase = async (input, opts = {}) => {
  ...
  let examPackage;
  if (input.examPackageUuid) {
    examPackage = await ExamPackageRepository.findOne({
      uuid: input.examPackageUuid,
    });
    if (!examPackage) {
      return Response.formatServiceReturn(
        ...
        404,
        null,
        language.EXAM_PACKAGE_NOT_FOUND
      );
    }
  }
  ...
  if (input.essayPackageUuid) {
    essayPackage = await EssayPackageRepository.findOne({
    ...
    if (!essayPackage) {
      return Response.formatServiceReturn(
        ...
        404,
        null,
        language.ESSAY_PACKAGE.NOT_FOUND
      );
    }
  }
  ...
  if (!userPurchase) {
    return Response.formatServiceReturn(
      ...
      500,
      null,
      language.USER_PURCHASE.CREATE_FAILED
    );
  }
...
const claimUserPurchaseEssayPackage = async (input, opts = {}) => {
  ...
  if (userPurchase) {
    return Response.formatServiceReturn(
      ...
      400,
      null,
      language.USER_PURCHASE.ESSAY_PACKAGE_ALREADY_CLAIMED
    );
  }
  ...
  if (!userPurchase) {
    return Response.formatServiceReturn(
      ...
      500,
      null,
      language.USER_PURCHASE.ESSAY_PACKAGE_CLAIM_FAILED
    );
  }
...
const deleteUserPurchase = async (input, opts = {}) => {
  ...
  return Response.formatServiceReturn(
    ...
    200,
    null,
    language.USER_PURCHASE.DELETE_SUCCESS
  );
};
